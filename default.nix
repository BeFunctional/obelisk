...
in rec {
  ...
  compressedJs = frontend: optimizationLevel: pkgs.runCommand "compressedJs" {} ''
    ...
    for f in **/all.js; do
      ...
      '' else ''
        '${pkgs.closurecompiler}/bin/closure-compiler' --externs '${reflex-platform.ghcjsExternsJs}' -O '${optimizationLevel}' --warning_level=QUIET --jscomp_warning=checkVars --create_source_map="$dir/all.js.map" --source_map_format=V3 --js_output_file="$dir/all.js" "$dir/all.unminified.js"
